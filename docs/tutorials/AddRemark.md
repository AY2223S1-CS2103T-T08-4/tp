...
This command allows users of the SETA application to add optional remarks to people in their address book and edit it if required. The command should have the following format:
...
Now that we have our `RemarkCommand` ready to be executed, we need to update `SETAParser#parseCommand()` to recognize the `remark` keyword. Add the new command to the `switch` block by creating a new `case` that returns a new instance of `RemarkCommand`.
...
:information_source: Don’t forget to update `SETAParser` to use our new `RemarkCommandParser`!
...
SETA stores data by serializing `JsonAdaptedPerson` into `json` with the help of an external library — Jackson. Let’s update `JsonAdaptedPerson` to work with our new `Person`!

While the changes to code may be minimal, the test data will have to be updated as well.

<div markdown="span" class="alert alert-warning">

:exclamation: You must delete SETA’s storage file located at `/data/addressbook.json` before running it! Not doing so will cause SETA to default to an empty address book!
...
This concludes the tutorial for adding a new `Command` to SETA.
